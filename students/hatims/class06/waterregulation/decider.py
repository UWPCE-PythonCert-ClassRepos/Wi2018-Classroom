...
class Decider(object):
    ...
    def decide(self, current_height, current_action, actions):
        ...
        action = None
        margin_region_low = self.target_height - (self.target_height * self.margin)
        margin_region_high = self.target_height + (self.target_height * self.margin)
        if (actions['PUMP_OFF'] == current_action) and (current_height < margin_region_low):
            action = actions['PUMP_IN']
        elif (actions['PUMP_OFF'] == current_action) and (current_height > margin_region_high):
            action = actions['PUMP_OUT']
        elif (actions['PUMP_OFF'] == current_action) and ((current_height >= \
            margin_region_high) or (current_height <= margin_region_low)):
            action = actions['PUMP_OFF']
        elif (actions['PUMP_IN'] == current_action) and (current_height > margin_region_high):
            action = actions['PUMP_OFF']
        elif (actions['PUMP_IN'] == current_action) and (current_height < margin_region_high):
            action = actions['PUMP_IN']
        elif (actions['PUMP_OUT'] == current_action) and (current_height < margin_region_low):
            action = actions['PUMP_OFF']
        elif (actions['PUMP_OUT'] == current_action) and (current_height > margin_region_low):
            action = actions['PUMP_OUT']
