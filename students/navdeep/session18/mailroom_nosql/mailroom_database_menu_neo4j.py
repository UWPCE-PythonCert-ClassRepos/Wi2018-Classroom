import os.path
from populate_mailroom_neo4j import ManipulateDBNeo
...
def addNewDonation(donor_collection_obj, first_name, last_name, email):
    ...
    donor_collection_obj.add_donor_neo(first_name, last_name, email, new_donation)
...
def delete_instance(donor_collection_obj):
    ...
    while invalid == True:
        try:
            ...
            if del_email == "" or not donor_collection_obj.donor_exists_neo(del_email):
                raise ValueError
        except ValueError:
            print("\nEmail does not exist in database.\n")
        else:
            invalid = False
    donor_collection_obj.delete_donation_neo(del_email)
...
url = input("URL: ") 
store_donors = ManipulateDBNeo(user, password, url)
