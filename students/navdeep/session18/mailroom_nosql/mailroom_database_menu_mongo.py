import os.path
from populate_mailroom_nosql import ManipulateDBMongo
...
def addNewDonation(donor_collection_obj, first_name, last_name, email):
    ...
    donor_collection_obj.add_donor_mongo(first_name, last_name, email, new_donation)
...
def delete_instance(donor_collection_obj):
    ...
    while invalid == True:
        try:
            ...
            if del_email == "" or not donor_collection_obj.donor_exists_mongo(del_email):
                raise ValueError
        except ValueError:
            print("\nEmail does not exist in database.\n")
        else:
            invalid = False
    donor_collection_obj.delete_donation_mongo(del_email)
...
store_donors = ManipulateDBMongo()
