...
class Controller(object):
    ...
    def tick(self):
        ...
        On each call to tick, the controller shall:
          ...
          3. query the decider for the next appropriate
          state of the pump, given the above
          4. set the pump to that new state
          :return: True if the pump has acknowledged
          its new state, else False
        """
        current_height = self.sensor.measure()
        current_state = self.pump.get_state()
        new_state = self.decider.decide(current_height, current_state, self.actions)
        self.pump.set_state(new_state)
        return self.pump.get_state == new_state
