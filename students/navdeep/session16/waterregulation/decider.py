...
class Decider(object):
    ...
    def __init__(self, target_height, margin):
        ...
        :param margin: the margin of liquid above
         and below the target height for which
         the pump should not turn on. Ex: .05 represents a
    ...
    def decide(self, current_height, current_action, actions):
        """
        Decide a new action for the pump, given the
        current height of liquid in the
        tank and the current action of the pump.
        Note that the new action for the pump MAY be
        ...
        one of actions['PUMP_IN'],
        actions['PUMP_OUT'],
        actions['PUMP_OFF']
        """
        max_height_allowed = (1 + self.margin) * self.target_height
        min_height_allowed = (1 - self.margin) * self.target_height
        new_action = None
        if current_action == actions['PUMP_OFF']:
            if current_height < min_height_allowed:
                new_action = actions['PUMP_IN']
            elif current_height > max_height_allowed:
