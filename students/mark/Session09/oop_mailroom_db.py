...
The script should accomplish the following goals:
    ...
    1. Send a Thank You 2. Create a Report or 3. quit
...
class DonorDB(object):
    ...
    def report_sort_key(self, item):
        """list.sort(key="<thing>") thing must be a function
        """
        return item[1]


    def print_donor_report(self, donor_database):
        """
    ...
    def add_donor_info(self, name, donor_db):
        ...
        print(self.ltr.create_letter(name, donor_db))


    def add_new_donor(self, donor_db):
        name = input("Enter a donor's name "
                     "(or 'list' to see all donors or 'menu' to exit)> ").strip()

        self.add_donor_info(name, self.donor_db)
